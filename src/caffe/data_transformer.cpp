...
template<typename Dtype>
void DataTransformer<Dtype>::Transform(IplImage* cv_img,const Datum* datum, Blob<Dtype>* transformed_blob){
  if (phase_==TRAIN){
...
template<typename Dtype>
void DataTransformer<Dtype>::Transform( IplImage* cv_img,
                                       Blob<Dtype>* transformed_blob) {
  // get the basic information of the image
  const int img_channels = cv_img->nChannels;
  const int img_height = cv_img->height;
  const int img_width = cv_img->width;

  //get the basic information of the transformed_blob
  const int channels = transformed_blob->channels();
  const int height = transformed_blob->height();
  const int width = transformed_blob->width();
  const int num = transformed_blob->num();
  ...
  CHECK(cv_img->depth == 8) << "Image data type must be unsigned byte";
  ...
  if ( smooth_filtering && apply_smooth ) {
    ...
    cvSmooth(cv_img, cv_img, smooth_type, smooth_param1);
    if (debug_display && phase_ == TRAIN)
      cvSaveImage("Smooth.jpg", cv_img);
  }
  ...
  if ( contrast_adjustment && apply_contrast ) {
    ...
    if ( Rand() % 2 )
        beta = - beta;
    cvConvertScale(cv_img, cv_img, alpha, beta);
    if (debug_display)
      cvSaveImage("Contrast.jpg", cv_img);
  }
 ...
 /* int QF = 100;
  ...
  if ( jpeg_compression && apply_JPEG ) {
	...
	if (debug_display && phase_ == Caffe::TRAIN)
      cvSaveImage("JPEG Compression", img);
  }
    */
  ...
  //crop the image
  IplImage *img_crop_pad = cropPadImage(cv_img, roi_width, roi_height,
	                                    rng_w, rng_h, warp_fillval);
  if (debug_display)
      cvSaveImage("Cropping.jpg", img_crop_pad);
  ...
  for (int h = 0; h < height; ++h) {
    for (int w = 0; w < width; ++w) {
      for (int c = 0; c < img_channels; ++c) {
        ...
        data_index = h * step + w * channels + c; // the imageSize of IplImage is img->height $\times$ img->widthStep

        Dtype pixel = static_cast<Dtype>(data[data_index]);
        if (has_mean_file) {
         // The index of mean value at each pixel point is not strictly corresponding
