...
void CompactDataLayer<Dtype>::InternalThreadEntry() {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    string value = datum.data();
    LOG(INFO) << "Size of the value is "<< value.size();
    mat = Mat(datum.height(), datum.width(), CV_8UC3);
    uchar* p;
    for (int h =0 ; h <datum.height();h++){
        ...
        for (int w = 0;w <datum.width();w++){
            for (int c = 0; c <3; c++) {
                ...
                p[w*3+c] = static_cast<char>(value.c_str()[index]);
            }
        }
    }
    LOG(INFO) << int(value.c_str()[0]) <<int(value.c_str()[1]) << mat.depth()<<mat.channels();
    IplImage img = IplImage(mat);
    LOG(INFO) << "Here" << img.depth;
//    img = cvDecodeImage(&mat,0);
    ...
    this->data_transformer_->Transform(&img,&datum, &(this->transformed_data_));
    if (this->output_labels_) {
